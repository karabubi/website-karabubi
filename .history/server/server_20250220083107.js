...
db.authenticate()
  .then(() => {
    console.log("Database connected...");
    return db.sync({ force: false }); // Synchronize models with database
  })
...
app.get("/test-email", async (req, res) => {
  const mailOptions = {
    from: process.env.EMAIL_USER,
    to: "recipient@example.com", // Replace with valid email
    subject: "Test Email",
    text: "This is a test email from Nodemailer.",
  };
...
app.post("/api/auth/register", async (req, res) => {
  ...
  try {
    ...
    // Create new user
    const user = await User.create({ name, username, password: hashedPassword, email });
    ...
    const mailOptions = {
      ...
      to: email,
      subject: "Your Registration Details",
      text: `Username: ${username}\nPassword: ${password}`, // Avoid sending passwords in emails!
    };
